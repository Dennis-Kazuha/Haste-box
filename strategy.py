...
def add_indicators(df: pd.DataFrame, ma_fast: int = 10, ma_slow: int = 200) -> pd.DataFrame:
    ...
    df["ma55"]          = df["close"].rolling(55,       min_periods=55).mean()      # ★ 新增
    df[f"ma{ma_slow}"] = df["close"].rolling(ma_slow,  min_periods=ma_slow).mean()
    df["vol_ma20"]      = df["volume"].rolling(20,      min_periods=20).mean()      # ★ 新增
    return df
...
def mark_1012_candidates(df: pd.DataFrame, ma_fast: int = 10, ma_slow: int = 200) -> pd.DataFrame:
    ...
    cond_t0 = (
        (df["open"]  > df[maf])   &   # 開盤站上 MA10
        (df["close"] > df[maf])   &   # 收盤站上 MA10
        (df["close"] > df["ma55"])&   # ★ 收盤站上 MA55（中期生命線）
        (df["close"] > df[mas])   &   # 收盤站上 MA200
        (df["close"] > bh_t1)     &   # 收盤突破 T-1 實體高
        (df["volume"] > df["vol_ma20"])  # ★ 成交量 > 20日均量（帶量突破）
    )

    df["_1012_raw"] = cond_t2 & cond_t1 & cond_t0
...
def run_strategy(
    ...
    for c in ["signal_1012","speed_box_ok","speed_box_fail","sb_stop",
              "weekly_tp","weekly_sl","v_reload","v_blocked","in_position","sb_active"]:
        df[c] = False
    for c in ["entry_price","stop_loss_price","sb_param0","sb_param1","sb_target","sb_tp1","sb_tp2","sb_tp3"]:
        df[c] = np.nan
    ...
    for i in range(len(df)):
        ...
        if row["_wo_trigger"]:
            if state["in_position"]:
                df.at[idx, "weekly_tp" if close > state["entry_price"] else "weekly_sl"] = True
        ...
        if state["sb_active"] and state["in_position"]:
            ...
            df.at[idx, "sb_tp3"] = state["sb_p0"] + _dist * 5.8   # 與 sb_target 相同
        ...
        if (not state["in_position"] and not state["washout_locked"]
            ...
            state = _enter_position(state, close, low,
                                    float(row["_t1_low"]), float(row["_t1_high"]), sb_ratio)
        ...
        if state["v_monitor"] and not state["in_position"] and pd.notna(ma_val):
            ...
            if (not state["v_blocked"] and state["v_ref_price"] is not None
                ...
                state["entry_price"]    = state["v_ref_price"]
    ...
    df.drop(columns=["_1012_raw","_t1_low","_t1_high","_wo_trigger"],
            errors="ignore", inplace=True)

    # ── 極速框成立後的回踩偵測（需要 3D K 線）───────────────────────
    ...
    if three_d is not None and not three_d.empty:
        ...
        df["Daily_Trend_Up"]  = df["close"] > df[f"ma{ma_fast}"]
        df["3D_Trend_Up"]     = False
        df["Weekly_Trend_Up"] = False
        df["MTF_Score"]       = df["Daily_Trend_Up"].astype(int)
...
def detect_sb_pullback(
    ...
    for sb_date in sb_ok_dates:
        ...
        dead          = False
        entry_found   = False
        lows_in_pb    = []    # 記錄回踩期間每根 3D K 的最低點（用於底底高判斷）
...
def mark_on_deck(df: pd.DataFrame, ma_fast: int = 10, ma_slow: int = 200) -> pd.DataFrame:
    ...
    bh_prev  = df["body_high"].shift(1)   # 昨日實體高（T-1 壓回不能破的線）

    # 今日（扮演 T-1 角色）
    cond_t2 = c_prev > maf_prev                   # 昨日收盤站上 MA10

    cond_t1 = (
        (df["open"]  > df[maf]) &                 # 今日開盤在 MA10 之上
        (df["close"] > df[maf]) &                 # 今日收盤在 MA10 之上
        (df["close"] <= bh_prev)                  # 今日收盤未突破昨日實體高（壓回確認）
    )

    on_deck = cond_t2 & cond_t1

    # 明日觸發門檻 = max(今日MA10, 今日MA200, 今日實體高點)
    # 三者取最大，明日收盤必須同時超越這三條線才算 T-0 成立
...
def mark_mtf_trend(
    ...
    """
    計算三個時間框架趨勢狀態 + 3D 階梯支撐壓力，對齊至日線 DataFrame。

    新增欄位：
        Daily_Trend_Up  : bool  — 日線收盤 > 日線 MA10
        3D_Trend_Up     : bool  — 3D 收盤 > 3D MA10（ffill 對齊日線）
        Weekly_Trend_Up : bool  — 週線收盤 > 週線 MA10（ffill 對齊日線）
        MTF_Score       : int   — 三燈加總（0~3）
        Prev_3D_High    : float — 前一根 3D K 最高點（階梯壓力）
        Prev_3D_Low     : float — 前一根 3D K 最低點（階梯支撐）
    """
    maf = f"ma{ma_fast}"
    df  = daily.copy()
    ...
    # ── 3D 趨勢（ffill 對齊日線）─────────────────────────────────────
    td = three_d.copy()
    td["_3d_ma10"]  = td["close"].rolling(ma_fast, min_periods=1).mean()
    td["_3d_trend"] = td["close"] > td["_3d_ma10"]
    ...
    # ── 週線趨勢（ffill 對齊日線）─────────────────────────────────────
    wk = weekly.copy()
    wk["_wk_ma10"]  = wk["close"].rolling(ma_fast, min_periods=1).mean()
    wk["_wk_trend"] = wk["close"] > wk["_wk_ma10"]
    ...
    # ── 3D 階梯支撐壓力（前一根 3D K 高低點，ffill 對齊日線）─────────
...
def analyze_manual_position(
    ...
    # ── 停利：R 倍數（1:5 / 1:10 / 1:15 / 1:20）────────────────────
    R = entry_price - recommended_stop    # 每股風險金額
